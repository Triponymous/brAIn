...
// ── Regions positioned like a real human brain (sagittal view) ──
const REGIONS = [
  // Somatosensory cortex – top center, slightly posterior (postcentral gyrus)
  { id: "r_sensory", label: "Sensorik", desc: "Sinneseindruecke empfangen",
    color: "#6ee7b7", pos: [0, 55, -10] as const, dataKey: "sensory" as const, maxSpikes: 100 },

  // Temporal lobe – lateral and inferior (pattern recognition, auditory)
  { id: "r_feature", label: "Mustererkennung", desc: "Muster & Features erkennen",
    color: "#67e8f9", pos: [-35, -5, 15] as const, dataKey: "feature" as const, maxSpikes: 150 },

  // Parietal lobe – superior posterior (association, integration)
  { id: "r_association", label: "Verknuepfung", desc: "Informationen verbinden",
    color: "#c084fc", pos: [0, 50, -35] as const, dataKey: "feature" as const, maxSpikes: 200 },

  // Prefrontal cortex – anterior superior (executive function, concepts)
  { id: "r_concept", label: "Konzeptbildung", desc: "Abstrakte Konzepte bilden",
    color: "#34d399", pos: [0, 35, 55] as const, dataKey: "concept" as const, maxSpikes: 10 },

  // Hippocampus – deep medial temporal (memory formation)
  { id: "r_wm", label: "Gedaechtnis", desc: "Aktive Erinnerungen halten",
    color: "#a78bfa", pos: [0, -10, 5] as const, dataKey: null, maxSpikes: 1 },

  // Motor cortex – precentral gyrus, top center slightly anterior
  { id: "r_motor", label: "Motorik", desc: "Aktionen & Output steuern",
    color: "#fbbf24", pos: [0, 60, 5] as const, dataKey: null, maxSpikes: 1 },
];

// Sensors arrive from outside the brain (external stimuli)
const SENSORS = [
  { id: "s_app",   label: "App",      color: "#60a5fa", pos: [-70, 50, -60] as const },
  { id: "s_keys",  label: "Tastatur", color: "#34d399", pos: [-75, 35, -50] as const },
  { id: "s_mouse", label: "Maus",     color: "#a78bfa", pos: [-70, 20, -40] as const },
  { id: "s_idle",  label: "Idle",     color: "#fbbf24", pos: [-65,  5, -30] as const },
  { id: "s_mic",   label: "Mikrofon", color: "#f87171", pos: [-60, -10, -20] as const },
  { id: "s_time",  label: "Zeit",     color: "#818cf8", pos: [-55, -25, -10] as const },
];
...
function buildGraph(state: ExtendedState): { nodes: GNode[]; links: GLink[] } {
  ...
  REGIONS.forEach(r => {
    ...
    nodes.push({ id: r.id, type: "region", label: r.label, desc: r.desc, color: r.color,
      val: 20 + activity * 40, active: activity > 0.05, activity,
...
export function BrainPanel3D({ state }: { state: BrainState | null }) {
  ...
  // One-time setup: bloom + orbit camera
  useEffect(() => {
    const fg = fgRef.current;
    if (!fg || initDone.current) return;
    initDone.current = true;

    // Bloom – high threshold so only bright nodes glow, not the background
    try {
      const bloom = new UnrealBloomPass(
        new THREE.Vector2(dims.w || 800, dims.h || 500),
        1.2,   // strength
        0.6,   // radius
        0.3    // threshold – only things brighter than this glow
      );
      fg.postProcessingComposer().addPass(bloom);
    } catch (e) {
      console.warn("Bloom init failed, continuing without:", e);
    }

    // Camera: position to see the brain from front-left, slightly above
    fg.cameraPosition(
      { x: -80, y: 60, z: 120 },   // camera position
      { x: 0, y: 25, z: 0 },       // look-at (center of brain)
      0                              // instant
    );

    // Slow auto-orbit using the built-in controls
